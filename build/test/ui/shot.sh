...
#        do=list&t=<block type> | do=add | do=dev | do=zone&z=<zone> | do=pid&id=<poi id> | scroll=<px> (page) ss=<px> (sheet)
...
curl -s -o /dev/null http://localhost:8787/_w.html || { (node "$HERE/serve.mjs" >/dev/null 2>&1 &); sleep 1; }
CHROME="${CHROME:-/c/Program Files/Google/Chrome/Application/chrome.exe}"
"$CHROME" --headless=new --disable-gpu --hide-scrollbars --user-data-dir="$OUT/prof_$1_$RANDOM" --window-size=390,844 \
  --virtual-time-budget=12000 --screenshot="$OUT/$1.png" "http://localhost:8787/_w.html?$2" >/dev/null 2>&1
echo "$OUT/$1.png"
