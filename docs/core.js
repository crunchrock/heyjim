...
const BT = {
  ...
  office: { n: 'Plugged office', ic: '🔌', dur: 240, log: 'dev', m: isOffice, b: p => (/panera/i.test(p.n) ? 4 : p.sc === 'library' ? 2 : 0), caps: ['work_indoor'], hint: 'Panera / library: power + Wi-Fi. Do builds, downloads, uploads here.' },
  deep: { n: 'Deep work', ic: '🧠', dur: 270, log: 'dev', m: isWork, b: p => (/panera/i.test(p.n) ? 3 : p.sc === 'library' ? 3 : 0), caps: ['work_indoor'], hint: 'Serious Bad Shrooms output. Phone away.' },
  light: { n: 'Light work', ic: '💻', dur: 120, log: 'dev', m: p => isWork(p) || isWater(p), b: p => (isWater(p) ? wfBonus(p) / 2 : 0), caps: ['work_indoor', 'work_outdoors'], hint: 'Notes, email, small tasks.' },
