...
- **Specials at a glance.** Every special shows its exact window. One running right now lights the place up (amber, pulsing dot, "Happy hour now · until 8p"), Today has a strip of what's on now / starting within 3h, and Find → Specials now lists on-now + later today. Specials need `label` + `items` + `days` (+ `start`/`end`); a one-off event uses `date`; a special whose day is unknown is never shown as today's.
- **Night out** (Today → Find → 🌃 Night out, and Places → Night out): one lookup for any evening, Sunday included, with a day picker for the coming week: happy hours (on now / later), strip clubs with that day's buffets, drink specials and free-entry windows, goth nights, ladies' nights, good bars and arcades, nearest first. It's a lookup: the Bar night auto-pick rules (never a club, never Sunday) stay as they are. `nightOut()` in core.js builds it.
- **Let him verify anything.** Every place sheet shows "Unsure? Check the source ↗" (first cited source, else website, else Google Maps), and specials / buffet prices / TV features link their own source with posted + checked dates. Unknown hours link to the site to check. **Bars:** Mon–Thu surface dated specials, Fri/Sat any good dive bar, never Sunday night. Ladies' nights and gentlemen's clubs (`strip_club`) have their own lists. **Clubs are normal places, not a seedy corner** (he asked for this): they rank like any bar for a Bar night, their happy hours sit in the Happy hours list with everyone else's, and a club stands in as a Meal pick while a food deal is actually running (`foodSpecials()`; free lunch buffet, steak night, trucker breakfast) and is buried when it isn't. **Groceries:** Trader Joe's > Sprouts/Publix > Walmart > Whole Foods. **Gas:** Murphy USA first.
...
- **Browser research (his Chrome, Claude in Chrome extension)** reaches what scripts can't: planetfitness.com club pages (fetch `/clubs/fl` + each `/gyms/<slug>` from inside the PF tab; hours are under "Club Hours"; it rate-limits (429) past ~100 fast requests), Google search (fetch `/search?q=…` from a google.com tab and read the snippets, pace ~2 s) and Google Maps place pages. Tool output truncates around 1 KB, so write big results into the page (`document.body.innerHTML = '<article><pre>…'`) and read them with get_page_text. Chrome blocks pages from posting to localhost. Some domains need the extension's site permission (screenshots still work). Local city guides beat per-bar searching: The Duvalist (`duvalist.com/happy-hour?day=week`, curl works) lists Jacksonville happy hours with windows; venue pages carry address + coordinates in ld+json. Match venues by location/address, never by name alone (chains have several branches).
- **Google Maps place data** (hours, permanently-closed flag, rating, phone, website, exact pin) comes from the JSON the Maps web app loads: `https://www.google.com/search?tbm=map&hl=en&gl=us&q=<name, street, city, FL>`, which also works from Node `fetch` with a browser User-Agent. Area lists (every strip club around a zone) need the full `pb` parameter captured from a Maps tab. Private helpers live in `packs/_tools/` (`gm.mjs` lookups, `hours_import.mjs` / `clubs_import.mjs` turn results into packs, `fixh.mjs` repairs midnight closing times, which Google sends as an empty time). Match results by house number or distance plus name, never distance alone (Tampa's clubs sit 0.1 mi apart). **Rate limit:** about 700 Maps lookups plus ~60 browser searches in an hour got his whole IP a Google "unusual traffic" CAPTCHA (it also hits his own searches). Pace ≥ 1.5 s, stop at the first `/sorry/` page, never solve the CAPTCHA.
